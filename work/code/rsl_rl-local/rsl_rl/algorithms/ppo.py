...
class PPO:
    ...
    def process_env_step(
        ...
        # Configure timeout boundaries. If the final observation is unavailable,
        # discard only the timeout-causing transition and bootstrap the preceding
        # valid transition from its known next state.
        configure_timeout_transition(
            self.storage,
            self.transition,
            obs,
    ...
    def update(self) -> dict[str, float]:
        ...
        for batch in generator:
            ...
            surrogate_loss = masked_mean(torch.max(surrogate, surrogate_clipped), loss_valid)
