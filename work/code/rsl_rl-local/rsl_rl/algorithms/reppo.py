...
class REPPO:
    ...
    def __init__(
        ...
        actor_parameters = chain(self.actor.parameters(), (self.log_alpha_temp, self.log_alpha_kl))
    ...
    def update(self) -> dict[str, float]:
        ...
        self._old_actor.eval()
        critic_totals = {"value": 0.0, "value_prediction_error": 0.0, "target_saturation": 0.0, "critic_grad_norm": 0.0}
        actor_totals = {"surrogate": 0.0, "q_value": 0.0, "entropy": 0.0, "kl": 0.0, "actor_grad_norm": 0.0}
    ...
    def _update_critic(self, batch: RolloutStorage.Batch) -> dict[str, float]:
        ...
        values, logits = self.critic(batch.observations, batch.actions, return_logits=True)
        per_sample_loss = self._raw_critic.compute_loss(logits, batch.returns)
    ...
    def _update_actor(self, batch: RolloutStorage.Batch) -> dict[str, float]:
        ...
        try:
            ...
            entropy = -distribution.log_prob(actions)
            q_values = self.critic(batch.observations, actions).squeeze(-1)
            primary_actor_loss = -(q_values + self.alpha_temp.detach() * entropy)
            with torch.no_grad():
                self._old_actor(batch.observations, stochastic_output=True)
                old_distribution = self._old_actor.distribution
                old_actions = old_distribution.sample(torch.Size([self.num_kl_samples]))
    ...
    def load(self, loaded_dict: dict, load_cfg: dict | None, strict: bool) -> bool:
        ...
        if load_cfg is None:
            load_cfg = {"actor": True, "critic": True, "optimizer": True, "dual_variables": True, "iteration": True}
    ...
    def construct_algorithm(obs: TensorDict, env: VecEnv, cfg: dict, device: str) -> REPPO:
        ...
        alg_class: type[REPPO] = resolve_callable(cfg["algorithm"].pop("class_name"))
        actor_class: type[MLPModel] = resolve_callable(cfg["actor"].pop("class_name"))
        critic_class: type[ActionValueModel] = resolve_callable(cfg["critic"].pop("class_name"))
