...
class SPO:
    ...
    This implementation is intentionally standalone: it does not import or inherit
    from :class:`PPO`. The rollout, GAE, value loss, entropy bonus, RND,
    symmetry augmentation, checkpointing, compilation, and multi-GPU logic are
    copied from RSL-RL PPO v5.4.0. Only the policy surrogate objective is changed.

    Reference:
        - Xie et al. "Simple Policy Optimization." ICML 2025.
    """
    ...
    def __init__(
        ...
        if clip_param <= 0.0:
            raise ValueError(f"SPO clip_param/epsilon must be positive, got {clip_param}.")
    ...
    # GIỮ NGUYÊN TỪ PPO v5.4.0: rollout và lưu behavior-policy data.
    ...
    # GIỮ NGUYÊN TỪ PPO v5.4.0: xử lý transition, timeout, RND và reset hidden state.
    ...
    # GIỮ NGUYÊN TỪ PPO v5.4.0: GAE và advantage normalization.
    ...
    # Phần KHÔNG giữ nguyên là block "SPO surrogate loss" bên dưới.
    ...
    # GIỮ NGUYÊN TỪ PPO v5.4.0: train/eval, save/load, export và compile.
    ...
    # GIỮ NGUYÊN TỪ PPO v5.4.0: factory construction; chỉ đổi type/name PPO -> SPO.
    @staticmethod
    def construct_algorithm(obs: TensorDict, env: VecEnv, cfg: dict, device: str) -> SPO:
        """Construct the SPO algorithm."""
        # Resolve class callables
        alg_class: type[SPO] = resolve_callable(cfg["algorithm"].pop("class_name"))  # type: ignore
        ...
        alg: SPO = alg_class(actor, critic, storage, device=device, **cfg["algorithm"], multi_gpu_cfg=cfg["multi_gpu"])
